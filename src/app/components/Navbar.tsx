...
export default function Navbar() {
  ...
  // Headroom-like behavior: hide on scroll down, show on scroll up
  useEffect(() => {
    ...
    function onScroll() {
      ...
      if (goingDown && !mobileOpen) setHidden(true);
  ...
  return (
    ...
    >
        ...
        <nav className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-3">
          <div className="flex items-center justify-between">
            ...
            <Link href="/" aria-label="HappySeraphim home" className="flex items-center gap-2">
              ...
              />
              <span className="font-semibold tracking-wide uppercase text-white text-sm sm:text-base">
                        ...
                        {item.items.map((sub) => (
                          <Link
                            ...
                            className={`block px-3 py-2 rounded-lg hover:bg-white/10 ${
                              pathname === sub.href ? "bg-white/10 text-white" : "text-white/90"
                            }`}
                          >
                            {sub.label}
                          </Link>
      ...
      {/* Mobile panel (drops under the fixed bar) */}
      {mobileOpen && (
        <div className="md:hidden absolute left-0 right-0 top-full px-4 pb-4">
